...
const actualizarCatalogo = async (articulos, moneda, proveedor) => {
    ...
    try {
        for (const art of articulos) {
            ...
            const undCaja = parseFloat(art.unidades_por_caja) || parseFloat(art.und_caja) || null;
            const derechos = parseFloat(art.derechos_porcentaje) || parseFloat(art.derechos) || null;
            ...
            if (existente) {
                ...
                await CatalogoArticulo.create({
                    ...
                    nombre: art.nombre,
                    proveedor: proveedor || '',
                    codigo_elaborador: art.codigo_proveedor || '',
                    moneda: moneda || '',
                    derechos_porcentaje: derechos,
                    imp_interno_porcentaje: impInterno,
                    unidades_por_caja: undCaja,
                    ultimo_valor_origen: valorOrigen,
                    ultimo_valor_fabrica: valorFabrica,
